...
if ( class_exists( 'MemberHero_Email', false ) ) {
	return;
}
 ...
 * MemberHero_Email class.
 */
class MemberHero_Email extends MemberHero_Settings_API {
	...
	public function __construct() {
		...
		if ( is_null( $this->template_base ) ) {
			$this->template_base = memberhero()->plugin_path() . '/templates/';
		}
		...
		add_action( 'memberhero_update_options_email_' . $this->id, array( $this, 'process_admin_options' ) );
	}
	...
	public function format_string( $string ) {
		...
		if ( has_filter( 'memberhero_email_format_string_replace' ) || has_filter( 'memberhero_email_format_string_find' ) ) {
			...
			$string = str_replace( apply_filters( 'memberhero_email_format_string_find', $legacy_find, $this ), apply_filters( 'memberhero_email_format_string_replace', $legacy_replace, $this ), $string );
		}

		/**
		 * Filter for main find/replace.
		 */
		return apply_filters( 'memberhero_email_format_string', str_replace( $find, $replace, $string ), $this );
	}
	...
	public function setup_locale() {
		if ( $this->is_user_email() && apply_filters( 'memberhero_email_setup_locale', true ) ) {
			memberhero_switch_to_site_locale();
		}
	}
	...
	public function restore_locale() {
		if ( $this->is_user_email() && apply_filters( 'memberhero_email_restore_locale', true ) ) {
			memberhero_restore_locale();
		}
	}
	...
	public function get_subject() {
		return apply_filters( 'memberhero_email_subject_' . $this->id, $this->format_string( $this->get_option( 'subject', $this->get_default_subject() ) ), $this->object );
	}
	...
	public function get_heading() {
		return apply_filters( 'memberhero_email_heading_' . $this->id, $this->format_string( $this->get_option( 'heading', $this->get_default_heading() ) ), $this->object );
	}
	...
	public function get_recipient() {
		$recipient  = apply_filters( 'memberhero_email_recipient_' . $this->id, $this->recipient, $this->object );
	...
	public function get_headers() {
		...
		return apply_filters( 'memberhero_email_headers', $header, $this->id, $this->object );
	}
	...
	public function get_attachments() {
		return apply_filters( 'memberhero_email_attachments', array(), $this->id, $this->object );
	}
	...
	public function get_title() {
		return apply_filters( 'memberhero_email_title', $this->title, $this );
	}
	...
	public function get_description() {
		return apply_filters( 'memberhero_email_description', $this->description, $this );
	}
	...
	public function get_option( $key, $empty_value = null ) {
		...
		return apply_filters( 'memberhero_email_get_option', $value, $this, $value, $key, $empty_value );
	}
	...
	public function is_enabled() {
		return apply_filters( 'memberhero_email_enabled_' . $this->id, 'yes' === $this->enabled, $this->object );
	}
	...
	public function style_inline( $content ) {
		if ( in_array( $this->get_content_type(), array( 'text/html', 'multipart/alternative' ), true ) ) {
			ob_start();
			memberhero_get_template( 'emails/email-styles.php' );
			$css = apply_filters( 'memberhero_email_styles', ob_get_clean(), $this );
	...
	public function get_from_name() {
		$from_name = apply_filters( 'memberhero_email_from_name', get_option( 'memberhero_email_from_name' ), $this );
	...
	public function get_from_address() {
		$from_address = apply_filters( 'memberhero_email_from_address', get_option( 'memberhero_email_from_address' ), $this );
	...
	public function send( $to, $subject, $message, $headers, $attachments ) {
		...
		$message              = apply_filters( 'memberhero_mail_content', $this->style_inline( $message ) );
		$mail_callback        = apply_filters( 'memberhero_mail_callback', 'wp_mail', $this );
		$mail_callback_params = apply_filters( 'memberhero_mail_callback_params', array( $to, $subject, $message, $headers, $attachments ), $this );
	...
	public function init_form_fields() {
		$this->form_fields = array(
			...
			'subject'    => array(
				...
				'type'        => 'text',
				'desc_tip'    => true,
				'description' => sprintf( wp_kses_post( __( 'Available placeholders: %s', 'memberhero' ) ), '<code>' . implode( '</code>, <code>', array_keys( $this->placeholders ) ) . '</code>' ),
	...
	public function get_email_type_options() {
		$types = array( 'plain' => __( 'Plain text', 'memberhero' ) );

		if ( class_exists( 'DOMDocument' ) ) {
			$types['html']      = __( 'HTML', 'memberhero' );
			$types['multipart'] = __( 'Multipart', 'memberhero' );
		}
	...
	protected function save_template( $template_code, $template_path ) {
		if ( current_user_can( 'edit_themes' ) && ! empty( $template_code ) && ! empty( $template_path ) ) {
			$saved = false;
			$file  = get_stylesheet_directory() . '/' . memberhero()->template_path() . $template_path;
			...
			if ( ! $saved ) {
				$redirect = add_query_arg( 'memberhero_error', urlencode( __( 'Could not write to template file.', 'memberhero' ) ) );
	...
	public function get_theme_template_file( $template ) {
		return get_stylesheet_directory() . '/' . apply_filters( 'memberhero_template_directory', 'memberhero', $template ) . '/' . $template;
	}
	...
	protected function move_template_action( $template_type ) {
		...
		if ( ! empty( $template ) ) {
			...
			if ( wp_mkdir_p( dirname( $theme_file ) ) && ! file_exists( $theme_file ) ) {
				...
				$template_file = apply_filters( 'memberhero_locate_core_template', $core_file, $template, $this->template_base, $this->id );
				 ...
				 */
				do_action( 'memberhero_copy_email_template', $template_type, $this );

				?>
				<div class="updated">
					<p><?php echo esc_html__( 'Template file copied to theme.', 'memberhero' ); ?></p>
				</div>
				<?php
			}
		}
		...
		if ( $template ) {
			if ( ! empty( $template ) ) {
				...
				if ( file_exists( $theme_file ) ) {
					 ...
					 */
					do_action( 'memberhero_delete_email_template', $template_type, $this );
					?>
					<div class="updated">
						<p><?php echo esc_html__( 'Template file deleted from theme.', 'memberhero' ); ?></p>
					</div>
					<?php
				}
			}
	...
	protected function admin_actions() {
		...
		if (
			...
			if ( empty( $_GET['_memberhero_email_nonce'] ) || ! wp_verify_nonce( memberhero_clean( wp_unslash( $_GET['_memberhero_email_nonce'] ) ), 'memberhero_email_template_nonce' ) ) {
				wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'memberhero' ) );
			}

			if ( ! current_user_can( 'edit_themes' ) ) {
				wp_die( esc_html__( 'You don&#8217;t have permission to do this.', 'memberhero' ) );
			}

			if ( ! empty( $_GET['move_template'] ) ) {
				$this->move_template_action( memberhero_clean( wp_unslash( $_GET['move_template'] ) ) );
			}

			if ( ! empty( $_GET['delete_template'] ) ) {
				$this->delete_template_action( memberhero_clean( wp_unslash( $_GET['delete_template'] ) ) );
			}
		}
	}
	...
	public function admin_options() {
		...
		$this->admin_actions();
		?>
		<h2><?php echo esc_html( $this->get_title() ); ?> <?php memberhero_back_link( esc_html__( 'Return to emails', 'memberhero' ), admin_url( 'admin.php?page=memberhero-settings&tab=email' ) ); ?></h2>
		 ...
		 */
		do_action( 'memberhero_email_settings_before', $this );
		?>
		 ...
		 */
		do_action( 'memberhero_email_settings_after', $this );
		?>
		...
		if ( current_user_can( 'edit_themes' ) && ( ! empty( $this->template_html ) || ! empty( $this->template_plain ) ) ) {
			...
			<div id="template">
				<?php
				$templates = array(
					'template_html'  => esc_html__( 'HTML template', 'memberhero' ),
					'template_plain' => esc_html__( 'Plain text template', 'memberhero' ),
				);
				...
				foreach ( $templates as $template_type => $title ) :
					...
					$core_file     = $this->template_base . $template;
					$template_file = apply_filters( 'memberhero_locate_core_template', $core_file, $template, $this->template_base, $this->id );
					$template_dir  = apply_filters( 'memberhero_template_directory', 'memberhero', $template );
					?>
					<div class="template <?php echo esc_attr( $template_type ); ?>">
						...
						<?php if ( file_exists( $local_file ) ) : ?>
							<p>
								...
								<?php if ( is_writable( $local_file ) ) : ?>
									<a href="<?php echo esc_url( wp_nonce_url( remove_query_arg( array( 'move_template', 'saved' ), add_query_arg( 'delete_template', $template_type ) ), 'memberhero_email_template_nonce', '_memberhero_email_nonce' ) ); ?>" class="delete_template button">
										<?php esc_html_e( 'Delete template file', 'memberhero' ); ?>
									</a>
								<?php endif; ?>

								<?php
								/* translators: %s: Path to template file */
								printf( esc_html__( 'This template has been overridden by your theme and can be found in: %s.', 'memberhero' ), '<code>' . esc_html( trailingslashit( basename( get_stylesheet_directory() ) ) . $template_dir . '/' . $template ) . '</code>' );
								?>
							</p>
						...
						<?php elseif ( file_exists( $template_file ) ) : ?>
							<p>
								...
								if ( is_writable( $target_dir ) ) :
									?>
									<a href="<?php echo esc_url( wp_nonce_url( remove_query_arg( array( 'delete_template', 'saved' ), add_query_arg( 'move_template', $template_type ) ), 'memberhero_email_template_nonce', '_memberhero_email_nonce' ) ); ?>" class="button">
										<?php esc_html_e( 'Copy file to theme', 'memberhero' ); ?>
									</a>
								<?php endif; ?>

								<?php
								/* translators: 1: Path to template file 2: Path to theme folder */
								printf( esc_html__( 'To override and edit this email template copy %1$s to your theme folder: %2$s.', 'memberhero' ), '<code>' . esc_html( plugin_basename( $template_file ) ) . '</code>', '<code>' . esc_html( trailingslashit( basename( get_stylesheet_directory() ) ) . $template_dir . '/' . $template ) . '</code>' );
								?>
							</p>
						...
						<?php else : ?>
							<p><?php esc_html_e( 'File was not found.', 'memberhero' ); ?></p>
						<?php endif; ?>
					</div>
				<?php endforeach; ?>
			</div>
			...
			memberhero_enqueue_js(
				...
				var view = '" . esc_js( __( 'View template', 'memberhero' ) ) . "';
				var hide = '" . esc_js( __( 'Hide template', 'memberhero' ) ) . "';
				...
				jQuery( 'a.delete_template' ).click( function() {
					if ( window.confirm('" . esc_js( __( 'Are you sure you want to delete this template file?', 'memberhero' ) ) . "') ) {
						return true;
					}
